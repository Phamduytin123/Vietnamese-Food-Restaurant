...
export class RecipeController {
    ...
    async recommendRecipes(
        ...
        return await this.recipeService.getRecommendations(lang, inputFeatures, listIngredients);
    }

    @Post('recognize')
    @UseInterceptors(FileInterceptor('file')) // Lấy file từ request
    async recognizeImage(@Lang() lang: string, @UploadedFile() file: Express.Multer.File) {
        if (!file) {
